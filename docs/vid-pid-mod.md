...
**Reference** — The whole job at a glance
```text
...
**WSL** — Install the tools you need
```bash
sudo apt install -y binwalk python3 pipx mtd-utils
```

**WSL** — Let Ubuntu find programs installed by pipx
```bash
pipx ensurepath
```
...
**WSL** — Download Jefferson
```bash
git clone https://github.com/onekey-sec/jefferson.git
```

**WSL** — Go into the Jefferson folder and install it
```bash
cd jefferson
poetry install
```
...
**WSL** — Swap in the CarlinKit version
```bash
...
**WSL** — Go back to your home folder
```bash
cd ~
```
...
**WSL** — Check both reads match and the size is right
```bash
cd ~
...
**WSL** — Make a backup, then delete the second read
```bash
cd ~
...
**WSL** — Cut the filesystem out of dump.bin
```bash
...
**WSL** — Go into the Jefferson folder
```bash
cd jefferson
```

**WSL** — Unpack the filesystem into extractedRootFS
```bash
...
**WSL** — Go back home and open the files in VS Code
```bash
cd ..
...
**WSL** — Fix the Exec format error for good
```bash
...
**Windows** — Restart WSL (run this in PowerShell)
```powershell
wsl --shutdown
```
...
**WSL** — Show the VID/PID lines you changed
```bash
cd ~
...
**Output** — What you should see (line numbers can be different)
```text
...
**WSL** — Pack the files into a new image
```bash
...
**WSL** — Check the size of the new image
```bash
...
**WSL** — Put the new image back into dump.bin
```bash
...
**WSL** — Delete the last dongle's files
```bash
...
**Reference** — `start_accessory.sh`: which line really runs
```text
...
**Reference** — `device_filter.xml`, turned from binary XML into readable text
```text
<resources>
